...
class plgButtonModulesAnywhere extends JPlugin
{
	...
	private function getHelper()
	{
		...
		if (
			...
			&& (
				(!is_array($params->components) && JFactory::getApplication()->input->get('option') == $params->components)
				|| (is_array($params->components) && in_array(JFactory::getApplication()->input->get('option'), $params->components))
			)
		)
		{
			return false;
