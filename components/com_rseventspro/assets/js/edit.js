...
function rs_check_repeat(val) {
	...
	if (val == 3)
		$('rs_repeat_on').style.display = '';
	else $('rs_repeat_on').style.display = 'none';
	
	createRepeats();
}

function rs_check_on(val) {
	if (val == 0) {
		$('repeat_on_day').style.display = 'none';
		$('repeat_on_day_order_container').style.display = 'none';
		$('repeat_on_day_type_container').style.display = 'none';
	} else if (val == 1) {
		$('repeat_on_day').style.display = '';
		$('repeat_on_day_order_container').style.display = 'none';
		$('repeat_on_day_type_container').style.display = 'none';
	} else {
		$('repeat_on_day').style.display = 'none';
		$('repeat_on_day_order_container').style.display = '';
		$('repeat_on_day_type_container').style.display = '';
	}
...
function createRepeats() {
	...
	if ($('repeat_end').value != '') {
		...
		selectedDays.each(function (el){
			params.push('days[]=' + el.value);
		});
		
		params.push('interval='+$('repeat_interval').value);
		params.push('type='+$('repeat_type').value);
		params.push('start='+$('start').value);
		params.push('end='+$('repeat_end').value);
		params.push('repeat_on_type='+$('repeat_on_type').value);
		params.push('repeat_on_day='+$('repeat_on_day').value);
		params.push('repeat_on_day_order='+$('repeat_on_day_order').value);
		params.push('repeat_on_day_type='+$('repeat_on_day_type').value);
		
		rse_root = typeof rsepro_root != 'undefined' ? rsepro_root : '';
		var repeatsRequest = new Request({
			method: 'post',
			url: rse_root + 'index.php?option=com_rseventspro',
			data: 'task=repeats&'+ params.join('&'),
