...
$sig(document).ready(function() {
	...
	// Drop-downs
	$sig('#language').chosen({'disable_search' : true});
	$sig('#sorting').chosen({'disable_search' : true});
	...
	if (editor !== '') {
		$sig('.sigProInsertButton').click(function(event) {
			...
			parent.jInsertEditorText('{gallery}' + gallery + ':' + width + ':' + height + ':' + displayMode + ':' + captionsMode + '{/gallery}', editor);
	...
	if ($sig("#sigProUploader").length > 0) {
		...
		// Language for labels
		$sig('#language').change(function(event) {
			event.preventDefault();
			window.location = 'index.php?option=com_sigpro&view=gallery&folder=' + $sig('input[name=folder]').val() + '&language=' + $sig(this).val() + '&type=' + $sig('input[name=type]').val() + '&tmpl=' + $sig('input[name=tmpl]').val() + '&editorName=' + $sig('input[name=editorName]').val();
		});
	}
});
