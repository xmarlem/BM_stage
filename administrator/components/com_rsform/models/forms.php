...
class RSFormModelForms extends JModelLegacy
{
				...
				foreach ($components as $component) {
					if (!empty($data[$component->ComponentId])) {
						...
						if (isset($properties['NAME'])) {
							...
							if ((isset($properties['REQUIRED']) && $properties['REQUIRED'] == 'YES') || // Add required fields
								(isset($properties['IMAGETYPE']) && $properties['IMAGETYPE'] != 'INVISIBLE') || // Add CAPTCHA fields
	...
	function getEmailLang($id = null)
	{
		...
		if ($lang = JFactory::getApplication()->input->getCmd('ELanguage')) {
			$session->set('com_rsform.emails.'.$cid.'.lang', $lang);
		}
		
		return $session->get('com_rsform.emails.'.$cid.'.lang', $this->getLang());
	}
