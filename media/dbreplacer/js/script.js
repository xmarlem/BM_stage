		...
		nnDBReplacer = {
			...
			initialize: function() {
				...
				this.fnckey = ( function() {
					self.pending++;
					window.setTimeout(function() { self.fnc(1); }, 500);
				} );
				...
				this.overlay = $('<div/>', {
					...
					zIndex: 5000,
					opacity: .2
				}).hide().on('click', function() { self._finishLoad(); }).appendTo('body');
			...
			getXML: function(field, params) {
				...
				$.ajax({
					type: 'post',
					url: 'index.php?nn_qp=1&folder=administrator.components.com_dbreplacer&file=dbreplacer.inc.php&field=' + field + params,
			...
			_updateField: function(type, clear) {
				...
				for (key in this.params) {
					val = encodeURIComponent(this.params[key]);
